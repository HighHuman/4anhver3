...
const emits = defineEmits(["searchDatas", "exportDatas"]);
...
const timeRangeSelected = ref<number>(30);
...
const searchData = () => {
  ...
  if (!isDisableQuickTime.value) {
    endTime = moment().valueOf();
    startTime = moment().add(-timeRangeSelected.value, "minute").valueOf();
  } else {
    startTime = moment(timeRangePicker.value[0]).valueOf();
    endTime = moment(timeRangePicker.value[1]).valueOf();
  }
...
<template>
  <div class="searchbox">
    ...
    <div class="btn search-btn" @click="searchData">
